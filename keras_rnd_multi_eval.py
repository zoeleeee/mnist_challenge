...
while True:
  if np.mean(tot_advs_acc) == 1.: 
    ...
    print('{} error rate per time: {:.2f}%; right rate: {:.2f}%; total adversarial acc:{}%'.format(tot_amt, np.sum(preds_dist[preds!=y_test] <= t)/len(preds), np.sum(preds_dist[preds==y_test] <= t)/len(preds), np.mean(tot_advs_acc)/len(preds))*100)
