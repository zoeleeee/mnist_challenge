...
class EAD(object):
  def __init__(self, sess, model, beta, decision_rule, batch_size,
    ...
    self.z = tf.Variable(
        np.zeros(self.z_shape), dtype=tf_dtype, name='z')
    ...
    self.setter_z = tf.assign(self.z, tf.map_fn(lambda x: self.rnd[int(x)], 
      tf.cast(tf.round(tf.multiply(self.slack, tf.cast(255, tf_dtype))), tf.int32)))
    ...
    if self.TARGETED:
      ...
      loss1_y = tf.multiply(tf.reduce_mean(tf.reduce_sum(tf.nn.sigmoid_cross_entropy_with_logits(self.output_y, self.tlab), axis=-1)), tf.cast(-1, tf_dtype))
    ...
    var_list = [self.global_step]+[self.slack]+[self.newimg]+[self.z]+new_vars
